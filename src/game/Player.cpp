...
void Player::Regenerate(Powers power)
{
    ...
    switch (power)
    {
        case POWER_MANA:
        {
            ...
            if (recentCast) // BlizzLike Updates Mana in intervals of 2s, which is correct
...
bool Player::CanCompleteQuest(uint32 quest_id)
{
    ...
    if (qInfo->HasFlag(QUEST_BLIZZLIKE_FLAGS_DELIVER))
    {
    ...
    if (qInfo->HasFlag(QUEST_BLIZZLIKE_FLAGS_KILL_OR_CAST | QUEST_BLIZZLIKE_FLAGS_SPEAKTO))
    {
        for (int i = 0; i < QUEST_OBJECTIVES_COUNT; ++i)
        {
    ...
    if (qInfo->HasFlag(QUEST_BLIZZLIKE_FLAGS_EXPLORATION_OR_EVENT) && !q_status.m_explored)
        return false;

    if (qInfo->HasFlag(QUEST_BLIZZLIKE_FLAGS_TIMED) && q_status.m_timer == 0)
        return false;
...
bool Player::CanCompleteRepeatableQuest(Quest const* pQuest)
{
    ...
    if (pQuest->HasFlag(QUEST_BLIZZLIKE_FLAGS_DELIVER))
        for (int i = 0; i < QUEST_OBJECTIVES_COUNT; ++i)
...
void Player::AddQuest(Quest const* pQuest, Object *questGiver)
{
    ...
    if (pQuest->HasFlag(QUEST_BLIZZLIKE_FLAGS_KILL_OR_CAST | QUEST_BLIZZLIKE_FLAGS_SPEAKTO))
    {
        for (int i = 0; i < QUEST_OBJECTIVES_COUNT; ++i)
    ...
    uint32 qtime = 0;
    if (pQuest->HasFlag(QUEST_BLIZZLIKE_FLAGS_TIMED))
    {
...
{
    if (!m_timedquests.empty() && qInfo->HasFlag(QUEST_BLIZZLIKE_FLAGS_TIMED))
    {
        if (msg)
...
// not used in blizzlike, but used in scripting code
...
//not used in blizzlike, function for external script library
...
void Player::ItemAddedQuestCheck(uint32 entry, uint32 count)
{
    for (int i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        Quest const* qInfo = objmgr.GetQuestTemplate(questid);
        if (!qInfo || !qInfo->HasFlag(QUEST_BLIZZLIKE_FLAGS_DELIVER))
            continue;
...
void Player::KilledMonsterCredit(uint32 entry, uint64 guid)
{
    ...
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        {
            if (qInfo->HasFlag(QUEST_BLIZZLIKE_FLAGS_KILL_OR_CAST))
            {
...
void Player::SendQuestReward(Quest const* pQuest, uint32 XP, Object * questGiver)
{
    ...
    gameeventmgr.HandleQuestgiverCompleteQuest(questid);
...
void Player::_LoadInventory(QueryResult_AutoPtr result, uint32 timediff)
{
    ...
    if (result)
    {
        ...
        {
            std::string subject = GetSession()->GetBlizzLikeString(LANG_NOT_EQUIPPED_ITEM);
    ...
    if (result)
    {
        do
        {
            ...
            if (pQuest)
            {
                ...
                if (pQuest->HasFlag(QUEST_BLIZZLIKE_FLAGS_TIMED) && !GetQuestRewardStatus(quest_id) &&  questStatusData.m_status != QUEST_STATUS_NONE)
                {
                    AddTimedQuest(quest_id);
...
bool Player::Satisfy(AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!isGameMaster() && ar)
    {
        ...
        if (LevelMin || LevelMax || missingItem || missingKey || missingQuest || missingHeroicQuest)
        {
            ...
            {
                if (missingItem)
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetBlizzLikeString(LANG_LEVEL_MINREQUIRED_AND_ITEM), ar->levelMin, objmgr.GetItemPrototype(missingItem)->Name1);
                else if (missingKey)
                    ...
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetBlizzLikeString(LANG_LEVEL_MINREQUIRED), LevelMin);
            }
            return false;
        }
    }
